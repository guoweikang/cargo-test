...
## 🎯 Zero build.rs Required!

Unlike traditional approaches, cargo-kbuild **doesn't require build.rs in every crate**.

### Automatic Configuration Declaration
...
cargo-kbuild automatically:

1. **Scans** all workspace crates for `CONFIG_*` features
2. **Generates** `.cargo/config.toml` with all declarations:
...
rustflags = [
    ...
    # ... all CONFIG_* from workspace
]
```

3. **Compiles** with zero warnings!

### Why This Approach?

| Traditional (build.rs) | cargo-kbuild (.cargo/config.toml) |
|------------------------|-----------------------------------|
| Needs build.rs per crate | ❌ Zero build.rs files! |
| Manual maintenance | ✅ Auto-generated |
| Runs build scripts | ✅ Pure config, faster |
| Easy to get out of sync | ✅ Always in sync |

### For Developers

The `.cargo/config.toml` file is **auto-generated** and should not be committed to git. 

Just run `cargo-kbuild build` and it will be created automatically.
