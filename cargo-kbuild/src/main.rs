...
fn print_help() {
    ...
    println!("Kconfig-style configuration system for Cargo (transparent wrapper)");
    println!();
    println!("USAGE:");
    println!("    cargo-kbuild <COMMAND> [OPTIONS]");
    println!();
    println!("OPTIONS:");
    ...
    println!("    cargo-kbuild build --kconfig .config.debug");
    println!("    cargo-kbuild test --kconfig custom.config -- --nocapture");
    println!();
    println!("NOTES:");
    println!("    - cargo-kbuild is a transparent wrapper around cargo");
    println!("    - Any cargo command can be used (build, test, run, check, clippy, etc.)");
...
fn cmd_cargo_wrapper(cargo_cmd: &str, args: &[String], kconfig_path: Option<PathBuf>) {
    ...
    let config_path = kconfig_path
        .unwrap_or_else(|| workspace_root.join(".config"));
    
    // Apply kbuild configuration (same as build command)
    if let Err(e) = apply_kbuild_config(&workspace_root, &config_path, cargo_cmd, args) {
        eprintln!("❌ Error: {}", e);
        process::exit(1);
    }
}
...
fn main() {
    ...
    match remaining_args.get(0).map(|s| s.as_str()) {
        ...
        Some(cmd) => {
            ...
            cmd_cargo_wrapper(cmd, &remaining_args[1..], kconfig_path);
        }
        None => {
